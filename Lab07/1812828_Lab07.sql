...
CREATE DATABASE 1812828_Database_Lab07;
USE 1812828_Database_Lab07;
...
ALTER TABLE employees ADD FULLTEXT(firstName);
...
where match(productName) against('Suzuki');
-- c. Tìm các sản phẩm có chứa cả hai từ “1985” và “Supra”
select*from products 
where match(productName) against('+1985 +Supra');

-- d. Tìm các có chứa từ “Ford” nhưng không có từ “truck”
alter table products add fulltext(productName);
select*from products
where match(productName) against('+Ford -truck' in boolean mode);

-- e. Tìm khách hàng có tên bắt đầu bằng từ “Au”
alter table customers add fulltext(customerName);
select*
from customers 
where match(customerName) against('Au*' in boolean mode);

-- f. Tìm các sản phẩm có chứa “1999” đồng thời với các sản phẩm có từ
-- “Yamaha” thì kết quả trả về trước tiên.
alter table products add fulltext(productName);
select *
from products
where match(productName) against('1999 +yamaha' in boolean mode);
