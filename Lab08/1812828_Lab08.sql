...
CREATE DATABASE 1812828_Database_Lab08;
USE 1812828_Database_Lab08;
...
(1143,'
','Anthony','x5428','abow@classicmodelcars.com','1',1056,'Sales Manager (NA)'),
...
DELIMITER ;
call GetFirst();
-- drop procedure GetFirst;
-- b. Đổi tên nhân viên có lastName “Vanauf” thành “Trump”
DELIMITER $$
create procedure UpdateLastName()
begin
select replace(lastName,'Vanauf','Trump') as lastName, firstName
FROM employees;
END$$
DELIMITER 
call UpdateLastName();
...
begin
	select orders.status, orderDate
    from orders
...
drop procedure ListCustomerByNameEmployee;
-- e. Lấy thông tin các khách hàng mà đơn hàng đã bị hủy
DELIMITER $$
CREATE PROCEDURE GetThongTin()
BEGIN
select A.*, B.status
...
-- drop procedure GetThongTin;
-- f. Lấy thông tin phòng ban nhân viên dựa vào mã employeeNumber
DELIMITER $$
CREATE PROCEDURE GetEmployeesById(In idval int(11))
BEGIN
	select A.*, B.*
    from employees A, offices B
	where A.officeCode=B.officeCode and A.employeeNumber=idval;
	END; $$
DELIMITER ;
call GetEmployeesById('1002');
-- drop procedure GetEmployeesById;

-- g. Liệt kê các sản phẩm của một dòng sản phẩm
DELIMITER $$
CREATE PROCEDURE ProductByProductLine(In line varchar(50))
BEGIN
	select *
    from products 
	...
	END; $$
DELIMITER ;
call ProductByProductLine('Motorcycles');
drop procedure ProductByProductLine;
-- h. Lấy ra các nhân viên của employeeNumber của nhân viên quản lý
DELIMITER $$
CREATE PROCEDURE GetEmployeesByEmloyeeNumber()
BEGIN
select *
from employees
...
DELIMITER ;
call GetEmployeesByEmloyeeNumber();
drop procedure GetEmployeesByEmloyeeNumber;
...
drop procedure ConvertProductLine;
-- j. Như yêu cầu trong câu h nhưng sử dụng CASE
DELIMITER $$
CREATE PROCEDURE GetEmloyeeNumberCase()
BEGIN
CASE jobTitle like '%Manager%' 
END CASE;
call GetEmployeesByEmloyeeNumberCase;
drop procedure GetEmployeesByEmloyeeNumberCase;
-- k. Lấy ra dòng sản phẩm dựa theo tên của một sản phẩm
DELIMITER $$
CREATE PROCEDURE productLineByproductName(In name varchar(70))
-- DROP PROCEDURE IF EXISTS productLineByproductName$$;
BEGIN
...
END$$
DELIMITER ;
insert into employees(employeeNumber, lastName, firstName) value(2000,'Vo Thi Thuy','Phuong');
