...
CREATE DATABASE 1812828_Database_Lab06;
USE 1812828_Database_Lab06;
...
-- ==========================Câu hỏi lab06 =========================
-- a. Lấy ra các sản phẩm có đơn đặt hàng trong tháng 5/2003.
select  products.productName, products.productLine,orders.orderDate
from products 
inner join orderdetails on products.productCode=orderdetails.productCode
inner join orders on orderdetails.orderNumber = orders.orderNumber
where month(orders.orderDate)=5 and year(orders.orderDate)=2003;

-- b. Đưa ra các thông tin về các đơn hàng trong các tháng của năm 2005
select* , month(orderDate)as Month
from orders
where year(orderDate)=2005
group by month(orderDate);
-- c. Đưa ra thông tin về các đơn hàng và tổng giá trị đơn hàng
select*, sum(priceEach*quantityOrdered)as total
from orderdetails
group by orderNumber
order by total asc;

-- d. Đưa ra thông tin về đơn hàng có tổng giá trị đơn hàng nhỏ nhất
select*, sum(priceEach*quantityOrdered)as total
from orderdetails
group by orderNumber
having total <= ALL(
...
-- e. Với mỗi khách hàng, đưa ra tổng số hàng, và tổng số tiền họ đã thanh toán
select products.productCode, customers.customerName, customers.customerNumber, sum(quantityInStock) ,
...
-- f. Liệt kê họ tên khách hàng cùng số lượng các đơn bị hủy của họ trong năm
-- 2003.
select customers.customerNumber,customers.contactLastName,customers.contactFirstName,
count(orders.status), orders.requiredDate
from customers
join orders on customers.customerNumber=orders.customerNumber 
where orders.status='Cancelled' and year(requiredDate)=2003
group by customers.contactLastName;
-- g. Đưa ra các mã đơn đặt hàng có giá trị lớn nhất không thuộc dòng sản phẩm
-- “Classic Cars”.
select orderdetails.orderNumber, orders.orderDate, products.productLine, sum(orderdetails.priceEach*orderdetails.quantityOrdered)as total
from orders
join orderdetails on orderdetails.orderNumber=orders.orderNumber
join products on products.productCode=orderdetails.productCode
...
-- l. Đưa ra thông tin về các mặt hàng có ít nhất một người đặt mua trong năm
-- 2005 nhưng không mua trong năm 2004.
select orders.*, count(orders.orderDate) total
from orderdetails
join orders on orderdetails.orderNumber=orders.orderNumber
where year(orders.orderDate)=2005 and year(orders.orderDate)<>2004
group by orders.orderNumber;
-- m. Đưa ra thông tin về các đơn hàng trong tháng 3/2005 và tổng giá trị của mỗi
...
group by orders.orderNumber;
-- n. Đưa ra thông tin về các dòng sản phẩm và các sản phẩm của từng dòng sản
-- phẩm đó. Sắp xếp giảm dần theo thứ tự abc.
select productCode, productLine, productName
from products
group by productLine
order by productLine asc;
-- o. Đưa ra thông tin về các sản phẩm không thuộc dòng sản phẩm Motorcycles
-- mà số ký tự trong tên sản phẩm không vượt quá 25.
select*
from products
where productLine <>'Motocrycles' and char_length(productName)<=25
group by productLine;
-- 2. Sử dụng truy vấn con không tương quan để thực hiện các yêu cầu truy vấn như
-- trong câu 1.
