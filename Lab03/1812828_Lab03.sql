...
CREATE DATABASE 1812828_Database_Lab03;
USE 1812828_Database_Lab03;
...
-- a. Lấy ra thông tin của các khách hàng sống tại các thành phố Nantes và Lyon.
...
select*from products 
where buyPrice BETWEEN 70 and 200;
-- d. Tìm tất cả các sản phẩm với giá mua nằm ngoài phạm vi 50$ và 150$
select*from products
where buyPrice NOT BETWEEN 50 and 150;

-- e. Tìm các đơn hàng đã được chuyển trong khoảng thời gian từ ‘15/1/2003’ đến ‘10/5/2003’.
select*from orders
where shippedDate between '2003/1/15' and '2003/5/10';

-- f. Tìm các đơn hàng đã được chuyển trước ngày ‘10/1/2003’ 1 tháng
SELECT * FROM orders where shippedDate <= DATE_SUB('2003/1/10', INTERVAL 30 DAY);
...
select*from products where productCode LIKE '%10';
-- l. Truy vấn 5 sản phẩm có số lượng trong kho là lớn nhất.
select*from products order by quantityInStock DESC LIMIT 5;
-- m. Đưa ra danh sách các sản phẩm và thêm thuộc tính là tiền hàng tồn của sản phẩm.
select  productCode, productName, quantityInStock * buyPrice AS productInventory from products;

-- n. Lấy ra thông tin về tên các sản phẩm theo thứ tự giảm dần của số lượng hàng tồn kho, tăng dần của giá sản phẩm
select*from products order by quantityInStock desc, buyPrice asc ;

-- o. Lấy ra thông tin về các sản phẩm và dòng sản phẩm (không sử dụng kết bảng)
